...
def check_weather_user_doesnt_exist(user_id: int, company_id: int) -> None:
    current_company_user = repo.check_weather_user_exists(user_id, company_id)
    assert not current_company_user, f'User with id {user_id} is already registered for the company with id {company_id}'

def check_weather_user_exists(user_id: int, company_id: int) -> CompanyUser:
    current_company_user = repo.check_weather_user_exists(user_id, company_id)
    assert current_company_user, f'User with id {user_id} is not registered for the company with id {company_id}'
    return current_company_user

def attach_user(user_id: int, company_id: int, company_status: CompanyStatus) -> None:
    check_weather_user_doesnt_exist(user_id, company_id)
...
def get_company_user_by_id(company_user_id: int, current_user_id: int) -> CompanyUser:
    ...
    check_weather_user_exists(current_user_id, company_user.company)
    return company_user
...
def update(company_user_id: int, company_user: CompanyUserUpdateForm, current_user_id: int) -> CompanyUser:
    ...
    check_weather_user_exists(current_user_id, company_user_to_update.company)
